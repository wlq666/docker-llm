...
app = FastAPI(title="Gemma",version='0.0.1',docs_url=None)
...
class Parameters(BaseModel):
    ...
    max_new_tokens: Optional[int] = Field(title="max new tokens", default=None, description="The maximum numbers of tokens to generate, ignoring the number of tokens in the prompt.")
    ...
    do_sample: Optional[bool] = Field(title="do sample", default=False, description="Whether or not to use sampling ; use greedy decoding otherwise.")
    ...
    temperature: Optional[float] = Field(title="temperature", default=1.0, description="The value used to modulate the next token probabilities. Increasing the temperature will make the model answer more creatively.")
    ...
    top_p: Optional[float] = Field(title="top p", default=1.0, description="If set to float < 1, only the smallest set of most probable tokens with probabilities that add up to `top_p` or higher are kept for generation.")
...
model_path = "/data/gemma/models"

model = GemmaModel(model_path)
...
def get_docs():
    return get_openapi(title="gemma",version='0.0.1', routes=app.routes)
