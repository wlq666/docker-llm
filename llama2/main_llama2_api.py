...
app = FastAPI(title="Llama2",version='0.0.1',docs_url=None)
...
class Parameters(BaseModel):
    ...
    max_new_tokens: Optional[int] = Field(title="max new tokens", default=4096, description="The maximum numbers of tokens to generate, ignoring the number of tokens in the prompt.")
    ...
    temperature: Optional[float] = Field(title="temperature", default=0.6, description="The value used to modulate the next token probabilities. Increasing the temperature will make the model answer more creatively.")
    ...
    top_p: Optional[float] = Field(title="top p", default=0.9, description="If set to float < 1, only the smallest set of most probable tokens with probabilities that add up to `top_p` or higher are kept for generation.")
...
def get_docs():
    return get_openapi(title="llama2",version='0.0.1', routes=app.routes)
