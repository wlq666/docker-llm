...
app = FastAPI(title="Qwen",version='0.0.1',docs_url=None)
...
class Parameters(BaseModel):
    ...
    top_k: Optional[int] = Field(title="top k", default=20, description="The number of highest probability vocabulary tokens to keep for top-k-filtering.")
    top_p: Optional[float] = Field(title="top p", default=0.8, description="If set to float < 1, only the smallest set of most probable tokens with probabilities that add up to `top_p` or higher are kept for generation.")
    repetition_penalty: Optional[float] = Field(title="repetition penalty", default=1.05, description="The parameter for repetition penalty. 1.0 means no penalty. See the paper for more details.")
